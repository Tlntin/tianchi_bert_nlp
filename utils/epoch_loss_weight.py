...
class EpochLossWeight(object):
    """
    用于动态调整loss的权重,每隔一个Epoch调整一次
    """
    def __init__(self):
    ...
    def get_f1_score_data():
        ...
        all_f1_score_dict = {}
        for file_name in config.file_name_list:
            file_path = os.path.join(config.out_data_dir, f'out_{file_name}.csv')
            out_df = pd.read_csv(file_path)
            epoch = len(out_df)
            if epoch == 1:
                f1_score = out_df.loc[epoch - 1, 'valid_f1_score']
                f1_score_list = [f1_score]
            else:
                f1_score_list = out_df.loc[epoch - 2:, 'valid_f1_score'].values.tolist()
    ...
    def run(self):
        ...
        """
        loss_weight_dict = {}
        if not os.path.exists(os.path.join(config.out_data_dir, 'out_average.csv')):
